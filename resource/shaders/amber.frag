...
vec4 effect(vec4 color, Image texture, vec2 uv, vec2 screenCoords)
{
    ...
    float textAlpha = Texel(texture, uv).r;
    ...
    innerGlow += Texel(texture, uv + vec2(0.001, 0.0)).r * 0.6;
    innerGlow += Texel(texture, uv - vec2(0.001, 0.0)).r * 0.6;
    innerGlow += Texel(texture, uv + vec2(0.0, 0.001)).r * 0.6;
    innerGlow += Texel(texture, uv - vec2(0.0, 0.001)).r * 0.6;

    // Outer glow effect** (minimal spread to avoid blurring)
    float outerGlow = textAlpha * 0.4;
    outerGlow += Texel(texture, uv + vec2(0.0025, 0.0)).r * 0.3;
    outerGlow += Texel(texture, uv - vec2(0.0025, 0.0)).r * 0.3;
    outerGlow += Texel(texture, uv + vec2(0.0, 0.0025)).r * 0.3;
    outerGlow += Texel(texture, uv - vec2(0.0, 0.0025)).r * 0.3;
    ...
    vec3 finalColor = backgroundColor + (amberColor * glow * scanlineStrength * flicker);

    // Ensure sharp, readable text by making alpha stronger
    return vec4(finalColor, clamp(textAlpha + glow * 0.5, 0.0, 1.0));
}