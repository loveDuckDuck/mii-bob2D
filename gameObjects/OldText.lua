...
OldText = GameObject:extend()

function OldText:new(area, x, y, opts)
    OldText.super.new(self, area, x, y, opts)
    self.font = GFont
    self.layer = 80
...
function OldText:draw()
    ...
    for i = 1, #self.characters do
        ...
        local length = GFont:getWidth(self.characters[i])

        love.graphics.print(self.characters[i], self.x + width * 5, self.y, 0, 5, 5, 0, self.font:getHeight() / 2)
    end
    love.graphics.setColor(GDefaultColor)
end

function OldText:destroy()
    OldText.super.destroy(self)
end
